...
func NewAnalyzer(dir string, lang Language) *Analyzer {
	return &Analyzer{
		workDir: dir,
		Modules: []*module.Module{
...
func (a *Analyzer) analyzeModule(mod *module.Module) {
	...
	for _, f := range mod.Files {
		s := scanner.New(a.lang, f)

		if config.DebugTokens {
			var tokens []token.Token
			tok := s.Next()
			for tok.ID != token.EOF {
			...
			l.Println(color.NewString("tokens for [%v]:", color.Red(f.Path)))
			w.Flush()
			l.Println()
		}

		p := parser.New(a.lang, s)
		p.Run()
